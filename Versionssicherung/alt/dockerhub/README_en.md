...
Grocylink extends [Grocy](https://grocy.info/) with automatic expiry alerts, multi-channel notifications, bidirectional CalDAV synchronization and a receipt scanner.
...
- **Receipt scanner**: Upload PDF receipts or use folder watching to extract products via OCR and book them as stock in Grocy
...
```bash
mkdir -p /path/to/grocylink/data /path/to/grocylink/receipts
chown 1000:1000 /path/to/grocylink/data /path/to/grocylink/receipts
```
...
4. **Receipts** (optional): Enable folder watching and adjust matching thresholds
