...
├── bring_runtime.py    # Dauerhafter Eventloop + gecachter Bring-Client
├── receipt_scanner.py  # Kassenbon-Pipeline (OCR, Parsing, Matching)
├── crypto.py           # Fernet-Verschluesselung fuer sensible Daten
├── requirements.txt    # Python-Dependencies
├── requirements-dev.txt # Zusaetzlich fuer die Tests (pytest)
├── pytest.ini          # Testkonfiguration
├── .dockerignore
├── tests/              # Testsuite (nicht im Docker-Image)
│   ├── conftest.py     # Attrappen fuer Bring-Client und Antworttypen
│   ├── test_bring_runtime.py
│   └── test_bring_sync.py
├── templates/
...
- `${PATH_TO}/grocylink/receipts/` - Kassenbon-PDFs fuer Ordnerueberwachung
...
| `volumes: ${PATH_TO}/...:/app/receipts` | Bind Mount fuer Kassenbon-PDFs (Ordnerueberwachung). |
...
Die Testsuite fuer den Bring!-Layer laeuft ohne Netzwerk und ohne
Datenbank - sie fasst weder die echte SQLite-Datei noch die Bring-API an.
...
```bash
python3 -m pytest --cov=bring_sync --cov=bring_runtime --cov-report=term-missing
```
...
Grocy (Shoppinglist | Volatile/Missing)
  ...
  -> Bring-Liste einmal laden + Match ueber UUID-Mapping bzw. Name
  -> Differenz sammeln (anlegen / aendern / entfernen)
  -> gebuendelt senden (batch_update_list, Bloecke zu 50)
...
#### Verbindungsmodell (seit 1.5.0)

Grocylink arbeitet synchron (Flask + APScheduler), die Bring-Library ist
asynchron. Beides verbindet `bring_runtime.py`: ein **dauerhafter
Eventloop** in einem Daemon-Thread, in dem aiohttp-Session und
eingeloggter Client zwischen den Aufrufen weiterleben.

- **Ein Login statt einem pro Request.** Bis 1.4.x oeffnete jeder Aufruf -
  Status, Listen, Listeninhalt, Sync - einen eigenen Eventloop mit eigenem
  Login. Ein Wechsel auf den Bring!-Tab kostete mehrere Anmeldungen.
- **Token-Erneuerung** uebernimmt `bring-api` selbst per Refresh-Token,
  solange der Client lebt. Laeuft der Token dennoch ab, wiederholt
  Grocylink den Aufruf einmal mit frischem Login.
- **Credential-Wechsel** wird ueber einen Hash der Zugangsdaten erkannt;
  beim Speichern neuer Daten wird der gecachte Client sofort verworfen.
- **Zeitlimit** von 180 Sekunden pro Aufruf, damit ein haengender Request
  keinen Gunicorn-Worker dauerhaft belegt.
- Jeder Gunicorn-Worker haelt seinen eigenen Client - bei mehreren Workern
  entsprechend mehrere Sitzungen.

Ein Sync kostet dadurch zwei Requests (Liste holen, Aenderungen senden)
statt frueher rund zwei pro Produkt.
...
- Beim Umbenennen eines Items legt Grocylink einen neuen Eintrag an und entfernt den alten. Ein reines Update per UUID waere moeglich, die Bring!-App zeigt den geaenderten Namen aber erst nach komplettem Reload an
- Die UUID neuer Eintraege vergibt Grocylink selbst. Das erspart den frueher noetigen Vollabruf der Liste nach jedem einzelnen neuen Item
...
2. **Fuzzy-Match** (automatisch): Falls keine gelernte Zuordnung existiert, wird per `token_sort_ratio` (rapidfuzz) das aehnlichste Grocy-Produkt gesucht (Quelle: `fuzzy`)
...
Unter **Kassenbons** > **Gelernte Zuordnungen** (aufklappbar) werden alle gespeicherten Zuordnungen angezeigt. Einzelne Zuordnungen koennen geloescht werden, um das Matching fuer bestimmte Bon-Produkte zurueckzusetzen.

#### Einstellungen

| Einstellung | Standard | Beschreibung |
|---|---|---|
| Ueberwachungsordner | `/app/receipts` | Ordner fuer automatische Bon-Erkennung |
| Scan-Intervall | `5` Minuten | Wie oft der Ordner auf neue PDFs geprueft wird |
| Match-Schwellwert | `70` % | Minimaler Score fuer Fuzzy-Match-Treffer |
| Auto-Confirm-Schwellwert | `95` % | Score ab dem Zuordnungen automatisch als korrekt gelten |
| Ordnerueberwachung | `aus` | Aktiviert/deaktiviert die periodische Ordnerueberwachung |
...
- `/opt/docker-data/grocylink/receipts/` - Kassenbon-PDFs fuer Ordnerueberwachung
...
6. **Nach Passwortwechsel**: Grocylink haelt den eingeloggten Client vor. Beim Speichern neuer Zugangsdaten wird er automatisch verworfen; im Zweifel den Container neu starten.
7. **"Bring-Anfrage ohne Antwort abgebrochen"**: Die Bring-API hat innerhalb von 180 Sekunden nicht geantwortet. Meist ein Netzwerkproblem oder eine Stoerung bei Bring! - der naechste geplante Sync versucht es erneut.

### Kassenbon-Scanner erkennt keine Produkte

1. **Digitaler PDF**: Pruefen ob der Bon tatsaechlich Textdaten enthaelt (nicht nur ein eingebettetes Bild). Im Container-Log wird die Extraktionsmethode angezeigt (`digital` oder `ocr`).
