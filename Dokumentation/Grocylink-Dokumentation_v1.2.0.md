...
Grocylink ist eine Webapplikation, die den Lagerbestand einer [Grocy](https://grocy.info/)-Instanz überwacht und automatisch Benachrichtigungen versendet, wenn Produkte bald ablaufen, bereits abgelaufen sind oder der Mindestbestand unterschritten wird. Zusätzlich synchronisiert Grocylink Tasks und Chores bidirektional mit einem CalDAV-Server und kann PDF-Kassenbons einlesen, Produkte extrahieren und als Bestand buchen.
...
- **Kassenbon-Scanner**: PDF-Kassenbons hochladen oder per Ordnerüberwachung einlesen, Produkte und Preise per OCR extrahieren, automatisch Grocy-Produkten zuordnen und als Bestand buchen
...
- `${PATH_TO}/grocylink/receipts/` - Kassenbon-PDFs für Ordnerüberwachung
...
> **Wichtig:** Die Datenverzeichnisse müssen für den konfigurierten User (UID/GID) schreibbar sein:
> ```bash
...
| `volumes: ${PATH_TO}/...:/app/receipts` | Bind Mount für Kassenbon-PDFs (Ordnerüberwachung). |
...
> **Wichtig:** Die Datenverzeichnisse müssen für den konfigurierten User schreibbar sein:
> ```bash
...
Grocylink kann PDF-Kassenbons einlesen, Produkte und Preise extrahieren und als Bestandsbuchungen in Grocy übernehmen. Sowohl digital erstellte PDFs als auch gescannte Kassenbons (per OCR) werden unterstützt.

#### Verarbeitungs-Pipeline

```
PDF (Upload / Ordnerüberwachung)
  ...
  → User prüft in UI (Zuordnungen korrigieren/bestätigen)
  → Bestaetigung → Bestand in Grocy buchen (add_stock)
  → Zuordnungen lernen (für zukünftige Bons)
```

#### Kassenbons hochladen

1. Navigiere zu **Kassenbons** in der Seitenleiste
2. PDFs per **Drag & Drop** in die Upload-Zone ziehen oder über **Dateiauswahl** hochladen
3. Der Kassenbon wird automatisch verarbeitet und erscheint in der Tabelle

#### Ordnerüberwachung

Grocylink kann einen Ordner periodisch auf neue PDF-Kassenbons überwachen:

1. Navigiere zu **Einstellungen** > **Kassenbons**
2. Trage den **Überwachungsordner** ein (Standard: `/app/receipts`)
3. Setze das **Scan-Intervall** (Standard: 5 Minuten)
4. Aktiviere die **Ordnerüberwachung** per Checkbox
5. Speichere die Einstellungen

Neue PDFs im Ordner werden automatisch erkannt, verarbeitet und in die Kassenbon-Liste übernommen. Bereits verarbeitete Dateien werden anhand des Dateipfads erkannt und nicht erneut eingelesen.

> **Volume-Mount:** Für die Ordnerüberwachung muss das Kassenbon-Verzeichnis als Volume gemountet sein:
> ```yaml
> volumes:
>   - ${PATH_TO}/grocylink/receipts:/app/receipts
> ```
...
1. Klicke auf **Prüfen** um den Review-Dialog zu öffnen
2. Alle erkannten Positionen werden mit ihrer automatischen Zuordnung angezeigt
3. Für jede Position wird ein **Match-Score** angezeigt:
   - **Grün (>90%)**: Hohe Übereinstimmung
   - **Gelb (70–90%)**: Mittlere Übereinstimmung — prüfen empfohlen
   - **Rot (<70%)**: Geringe Übereinstimmung — manuelle Korrektur nötig
4. Zuordnungen per **Dropdown** korrigieren (alle Grocy-Produkte durchsuchbar)
5. Mit **Bestätigen** werden alle Positionen als Bestand in Grocy gebucht

Bestätigte Zuordnungen werden automatisch gelernt und bei zukünftigen Kassenbons als erstes angewendet (exakter Match vor Fuzzy-Match).

#### Produkt-Matching

Das Matching erfolgt in zwei Stufen:

1. **Gelernte Zuordnungen** (exakt): Zuvor bestätigte Zuordnungen werden direkt angewendet (Score: 100, Quelle: `learned`)
2. **Fuzzy-Match** (automatisch): Falls keine gelernte Zuordnung existiert, wird per `token_sort_ratio` (rapidfuzz) das ähnlichste Grocy-Produkt gesucht (Quelle: `fuzzy`)
...
Unter **Kassenbons** > **Gelernte Zuordnungen** (aufklappbar) werden alle gespeicherten Zuordnungen angezeigt. Einzelne Zuordnungen können gelöscht werden, um das Matching für bestimmte Bon-Produkte zurückzusetzen.

#### Einstellungen

| Einstellung | Standard | Beschreibung |
|---|---|---|
| Überwachungsordner | `/app/receipts` | Ordner für automatische Bon-Erkennung |
| Scan-Intervall | `5` Minuten | Wie oft der Ordner auf neue PDFs geprüft wird |
| Match-Schwellwert | `70` % | Minimaler Score für Fuzzy-Match-Treffer |
| Auto-Confirm-Schwellwert | `95` % | Score ab dem Zuordnungen automatisch als korrekt gelten |
| Ordnerüberwachung | `aus` | Aktiviert/deaktiviert die periodische Ordnerüberwachung |
...
- `/opt/docker-data/grocylink/receipts/` - Kassenbon-PDFs für Ordnerüberwachung
...
| `DELETE` | `/api/receipts/<id>` | Kassenbon löschen |
| `PUT` | `/api/receipts/<id>/items/<item_id>` | Item-Zuordnung ändern |
| `POST` | `/api/receipts/<id>/confirm` | Kassenbon bestätigen und Bestand buchen |
| `POST` | `/api/receipts/<id>/reject` | Kassenbon verwerfen |
| `GET` | `/api/receipts/mappings` | Gelernte Zuordnungen abrufen |
| `DELETE` | `/api/receipts/mappings/<id>` | Gelernte Zuordnung löschen |
...
1. **Digitaler PDF**: Prüfen ob der Bon tatsächlich Textdaten enthält (nicht nur ein eingebettetes Bild). Im Container-Log wird die Extraktionsmethode angezeigt (`digital` oder `ocr`).
2. **Gescannter PDF**: Tesseract OCR benötigt eine ausreichende Bildqualität. Unscharfe oder schief gescannte Bons liefern schlechte Ergebnisse.
3. **Unbekanntes Bon-Format**: Das Parsing unterstützt gängige deutsche Kassenbon-Formate. Bei unbekannten Layouts werden möglicherweise keine Positionen erkannt. Im Container-Log erscheint der extrahierte Rohtext zur Diagnose.

### Kassenbon-Ordnerüberwachung funktioniert nicht

1. **Volume prüfen**: Ist `/app/receipts` als Volume gemountet? (`docker inspect grocylink | grep receipts`)
2. **Einstellungen prüfen**: Ordnerüberwachung muss in den Einstellungen aktiviert sein
3. **Berechtigungen**: Das Kassenbon-Verzeichnis muss für den Container-User lesbar sein:
   ```bash
...
```bash
# Datenverzeichnisse für den Container-User schreibbar machen
